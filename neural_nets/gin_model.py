...
Graph Isomorphism Network (GIN) implementation for Agent in PEARL.
...
MODEL STATS:
- Parameters: Comparable to GCN
- Architecture: 3 GIN layers + policy/value heads
"""
...
class GIN(nn.Module):
    """
    Graph Isomorphism Network for Tiramisu program optimization.
    
    Uses graph isomorphism operators to process program dependency graphs
    ...
    def __init__(
        ...
        num_gin_layers=3,    # Number of GIN layers
        ...
        # First GIN layer
        self.gin_layers.append(GINConv(nn.Linear(input_size, hidden_size), train_eps=True))
        if use_batch_norm:
        ...
        for i in range(1, num_gin_layers):
            self.gin_layers.append(GINConv(nn.Linear(hidden_size, hidden_size), train_eps=True))
            if use_batch_norm:
                self.batch_norms.append(BatchNorm1d(hidden_size))
            self.linear_projections.append(Linear(hidden_size, hidden_size))
        ...
        # Initialize GIN weights
        for gin_layer in self.gin_layers:
            for name, param in gin_layer.named_parameters():
                if "weight" in name:
    ...
    def shared_layers(self, data):
        ...
        # Process through each GIN layer
